...
/** bcrypt work factor — 12 is the recommended minimum for production (2024) */
...
export class UsersService {
  ...
  /**
   * updateRole is called by ADMIN only — adminId comes from the request context
   * stored in auditContextStorage via AuditHelper.
   */
...
export class UsersService {
  ...
  private async assertEmailUnique(email: string): Promise<void> {
    const exists = await this.userRepo.findOne({
      where: { email: email.toLowerCase().trim() },
      withDeleted: true, // also catch soft-deleted accounts
    });
    if (exists) {
      throw new ConflictException('An account with this email already exists');
    }
