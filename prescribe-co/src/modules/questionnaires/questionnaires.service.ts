...
export class QuestionnairesService {
  ...
  async update(id: string, dto: UpdateQuestionnaireDto): Promise<Questionnaire> {
    ...
    if (dto.questions !== undefined) {
      ...
      // Bump version when the schema itself changes
      const newVersion = questionnaire.version + 1;
      questionnaire.version = newVersion;
  ...
  /**
   * Validate answers against the live schema and persist an immutable response.
   * Returns the saved response with eligibility computed.
   */
  async submitResponse(
    questionnaireId: string,
    userId: string,
    ...
    // Validate + compute eligibility (throws BadRequestException on type errors)
    const result = this.validator.validate(questionnaire.schema, answers);
  ...
  /**
   * Retrieve the most recent eligible response for a user + questionnaire pair.
   * Used by the prescription module to validate eligibility before submission.
   */
  async findLatestEligibleResponse(
    userId: string,
    questionnaireId: string,
